        ...
        if (IS_POST) {
            ...
            if (! $verify->check($code)) {
                $output = array(
                    ...
                    'info' => urlencode('验证码错误！'),
                    'code' => - 202
                );
                exit(urldecode(json_encode($output)));
            }
            $roleModel = $role == 'A' ? 'admin' : ($role == 'M' ? 'mgrs' : '');
            // 实例化模型
            $users = M($roleModel);
            ...
            if (create_hash($password, $row['id_card_num']) == $row['password']) {
                // 针对管理员账户的状态进行判断
                if ($role == 'M' && $row['if_aprvd'] == 0) {
                    $output = array(
                        'data' => array(
                            'redirect_url' => urlencode($_SERVER['HTTP_HOST'] . __APP__ . '/Public/login'),
                            'sec' => 3
                        ),
                        'info' => urlencode('您的账户正在审核中，请耐心等待！'),
                        'code' => - 203
                    );
                    exit(urldecode(json_encode($output)));
                } elseif ($role == 'M' && $row['if_aprvd'] == - 1) {
                    $output = array(
                        'data' => array(
                            'redirect_url' => urlencode($_SERVER['HTTP_HOST'] . __APP__ . '/Public/login'),
                            'sec' => 3
                        ),
                        'info' => urlencode('您的账户审核不通过或被禁止访问，请联系管理员！'),
                        'code' => - 204
                    );
                    exit(urldecode(json_encode($output)));
                }
