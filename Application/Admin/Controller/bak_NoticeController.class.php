...
class NoticeController extends CommonController
{
    ...
    public function index_post()
    {
        if (IS_POST) {
            ...
            $show = $Page->show();
            // 实现数据分页
            $data = $notice->field('id,n_author,n_source,n_titile,n_valid_time,n_keywords,n_abstracts,n_create_date,n_apprv_time')
                ->where(array(
                'n_type' => $type,
                'n_status' => $status
            ))
    ...
    public function do_add()
    {
        if (IS_POST) {
            ...
            if (empty($data['n_type']) || empty($data['n_author']) || empty($data['n_source']) || empty($data['n_titile']) || empty($data['n_content']) || empty($data['n_keywords']) || empty($data['n_abstracts']) || empty($data['n_valid_time'])) {
                $output = array(
                    'data' => array(
                        'redirect_url' => urlencode($_SERVER['HTTP_HOST'] . __APP__ . '/Admin/Notice/add'),
                        'sec' => 3
                    ),
                    'info' => '通知信息不能为空！',
                    'code' => '-201'
    ...
    public function update_notice_list()
    {
        if (IS_POST) {
            ...
            $notice = D('notice');
            // 获取数据
            $data = $notice->field('id,n_author,n_source,n_titile,n_valid_time,n_content,n_keywords,n_abstracts,n_create_date,n_apprv_time')
                ->where(array(
                'n_id' => $id
            ))
                ->select();
    ...
    public function do_update()
    {
        if (IS_POST) {
            ...
            if ($notice->save($data)) {
                ...
                $output = array(
                    'data' => array(
                        'redirect_url' => urlencode($_SERVER['HTTP_HOST'] . __APP__ . '/Admin/Notice/update'),
                        'sec' => 3
                    ),
                    'info' => urlencode('更新通知失败！请重新再试！'),
                    'code' => - 200
                );
                exit(urldecode(json_encode($output)));
            }
