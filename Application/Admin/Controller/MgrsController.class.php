...
// 	}	
    ...
    public function edit_all(){
        if (IS_POST) {
            ...
            $id = I('post.user_id');
            $users = D('mgrs');
            $data = $users->field('id,icon_url,nick_name,true_name,gender,id_card_num,mobile,email')->where("id = '{$id}'")->find();
            //身份证号加*号隐藏部分信息
...
//     	        D('mgrs')->save($data);	        
    	        ...
    	        $confirm_password = MD5(strtolower(addslashes(trim(I('post.new_password2')))));
